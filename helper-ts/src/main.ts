...
function parsePatternToMinutes(pattern: string): { start: number | null; end: number | null } {
  const DAY_START = 0;
  const DAY_END = 24 * 60;
...
function createSlotRow(): HTMLDivElement {
  ...
  const removeBtn = row.querySelector('.remove-slot-btn') as HTMLButtonElement | null;
...
function updateSubmitButtonEnabled() {
  const helperNameInput = document.getElementById('helperName') as HTMLInputElement | null;
...
async function submitOneDay() {
  const helperName = (document.getElementById('helperName') as HTMLInputElement).value.trim();
  ...
  const allday = (row.querySelector('.allday') as HTMLSelectElement | null)?.value ?? '';
  const start = (row.querySelector('.start') as HTMLInputElement | null)?.value ?? '';
  const end = (row.querySelector('.end') as HTMLInputElement | null)?.value ?? '';
  const note = (row.querySelector('.note') as HTMLInputElement | null)?.value.trim() ?? '';
  ...
  const { start: startMinutes, end: endMinutes } = parsePatternToMinutes(pattern);
  ...
  if (payloads.length === 0) {
    ...
    if (delError) {
      ...
      statusEl.textContent = 'この日は「休み」として登録しました（希望スロットなし）✅';
    }
    return;
  }
...
async function loadOneDayFromSupabase() {
  ...
  const date = (document.getElementById('date') as HTMLInputElement).value;
  const statusEl = document.getElementById('status') as HTMLParagraphElement;
  const slotsContainer = document.getElementById('slotsContainer') as HTMLDivElement | null;
...
const { data, error } = await supabase
  ...
  for (const row of data) {
    ...
    const alldaySelect = slotRow.querySelector('.allday') as HTMLSelectElement | null;
    const startInput = slotRow.querySelector('.start') as HTMLInputElement | null;
...
async function loadMonthSummaryFromSupabase() {
  ...
  const dateInput = document.getElementById('date') as HTMLInputElement | null;
  const statusEl = document.getElementById('status') as HTMLParagraphElement;
  const summaryContainer = document.getElementById('summaryCards') as HTMLDivElement | null;
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const slotsContainer = document.getElementById('slotsContainer') as HTMLDivElement | null;
  const addSlotBtn = document.getElementById('addSlotBtn') as HTMLButtonElement | null;
  const submitBtn = document.getElementById('submitBtn') as HTMLButtonElement | null;
  const helperNameInput = document.getElementById('helperName') as HTMLInputElement | null;
  const dateInput = document.getElementById('date') as HTMLInputElement | null;
  const prevDayBtn = document.getElementById('prevDayBtn') as HTMLButtonElement | null;
  const nextDayBtn = document.getElementById('nextDayBtn') as HTMLButtonElement | null;
  const loadBtn = document.getElementById('loadBtn') as HTMLButtonElement | null;
  const summaryBtn = document.getElementById('loadMonthlySummaryBtn') as HTMLButtonElement | null;
