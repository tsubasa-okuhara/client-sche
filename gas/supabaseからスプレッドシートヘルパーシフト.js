...
function importHelperShiftsFromSupabase() {
  ...
  if (!(baseDate instanceof Date)) throw new Error('必要情報!B1 に日付が入っていません（例: 2026/01/01）');
  ...
  var lastDay = Utilities.formatString('%s-%02d', monthStr, getLastDayOfMonth_(year, month));

  var sheetName = 'ヘルパーシフト-' + monthStr;
  var sheet = ss.getSheetByName(sheetName) || ss.insertSheet(sheetName);
  sheet.clear();

  var header = ['日付', '曜日', 'ヘルパー名', 'パターン', '開始', '終了', '備考', '週'];
...
sheet.getRange(2, 4, 5000, 5).setNumberFormat('@');
  ...
  var url = config.url + '/rest/v1/helper_shift_requests'
    + '?date=gte.' + firstDay
    + '&date=lte.' + lastDay
